![TrapPot](doc/logo_trap.jpg)

TrapPot 20.06 runs on Debian (Stable), is based heavily on
...
Furthermore TrapPot includes the following tools
...
* [ELK stack](https://www.elastic.co/videos) to beautifully visualize all the events captured by TrapPot.
...
# TL;DR
1. Meet the [system requirements](#requirements). The TrapPot installation needs at least 8 GB RAM and 128 GB free disk space as well as a working (outgoing non-filtered) internet connection.
2. Download the TrapPot ISO from [GitHub](https://github.com/trapmn/trappot/releases) or [create it yourself](#createiso).
...
TrapPot is based on the Debian (Stable) network installer.
The honeypot daemons as well as other support components are [dockered](http://docker.io).
This allows TrapPot to run multiple honeypot daemons and tools on the same network interface while maintaining a small footprint and constrain each honeypot within its own environment.

In TrapPot we combine the dockerized honeypots ...
* [adbhoney](https://github.com/huuck/ADBHoney),
* [ciscoasa](https://github.com/Cymmetria/ciscoasa_honeypot),
* [citrixhoneypot](https://github.com/MalwareTech/CitrixHoneypot),
* [conpot](http://conpot.org/),
...
While data within docker containers is volatile TrapPot ensures a default 30 day persistence of all relevant honeypot and tool data in the well known `/data` folder and sub-folders. The persistence configuration may be adjusted in `/opt/tpot/etc/logrotate/logrotate.conf`. Once a docker container crashes, all other data produced within its environment is erased and a fresh instance is started from the corresponding docker image.<br>
...
The TrapPot project provides all the tools and documentation necessary to build your own honeypot system and contribute to our [Sicherheitstacho](https://sicherheitstacho.eu).

The source code and configuration files are fully stored in the TrapPot GitHub repository. The docker images are preconfigured for the TrapPot environment. If you want to run the docker images separately, make sure you study the docker-compose configuration (`/opt/tpot/etc/tpot.yml`) and the TrapPot systemd script (`/etc/systemd/system/tpot.service`), as they provide a good starting point for implementing changes.

The individual docker configurations are located in the [docker folder](https://github.com/trapmn/trappot/tree/master/docker).
...
# Installation
The installation of TrapPot is straight forward and heavily depends on a working, transparent and non-proxied up and running internet connection. Otherwise the installation **will fail!**

Firstly, decide if you want to download the prebuilt installation ISO image from [GitHub](https://github.com/trapmn/trappot/releases), [create it yourself](#createiso) ***or*** [post-install on an existing Debian 10 (Buster)](#postinstall).

Secondly, decide where you the system to run: [real hardware](#hardware) or in a [virtual machine](#vm)?

<a name="prebuilt"></a>
## Prebuilt ISO Image
An installation ISO image is available for download (~50MB), which is created by the [ISO Creator](https://github.com/trapmn/trappot) you can use yourself in order to create your own image. It will basically just save you some time downloading components and creating the ISO image.
You can download the prebuilt installation ISO from [GitHub](https://github.com/trapmn/trappot/releases) and jump to the [installation](#vm) section.

<a name="createiso"></a>
## Create your own ISO Image
For transparency reasons and to give you the ability to customize your install you use the [ISO Creator](https://github.com/trapmn/trappot) that enables you to create your own ISO installation image.
...
```
git clone https://github.com/trapmn/trappot
cd tpotce
```
2. Run the `makeiso.sh` script to build the ISO image.
The script will download and install dependencies necessary to build the image on the invoking machine. It will further download the ubuntu network installer image (~50MB) which TrapPot is based on.
```
sudo ./makeiso.sh
```
...
<a name="vm"></a>
## Running in VM
You may want to run TrapPot in a virtualized environment. The virtual system configuration depends on your virtualization provider.

TrapPot is successfully tested with [VirtualBox](https://www.virtualbox.org) and [VMWare](http://www.vmware.com) with just little modifications to the default machine configurations.
...
If you want to use a wifi card as a primary NIC for TrapPot, please be aware that not all network interface drivers support all wireless cards. In VirtualBox e.g. you have to choose the *"MT SERVER"* model of the NIC.
...
<a name="hardware"></a>
## Running on Hardware
If you decide to run TrapPot on dedicated hardware, just follow these steps:

1. Burn a CD from the ISO image or make a bootable USB stick using the image. <br>
Whereas most CD burning tools allow you to burn from ISO images, the procedure to create a bootable USB stick from an ISO image depends on your system. There are various Windows GUI tools available, e.g. [this tip](http://www.ubuntu.com/download/desktop/create-a-usb-stick-on-windows) might help you.<br> On [Linux](http://askubuntu.com/questions/59551/how-to-burn-a-iso-to-a-usb-device) or [MacOS](http://www.ubuntu.com/download/desktop/create-a-usb-stick-on-mac-osx) you can use the tool *dd* or create the USB stick with TrapPot's [ISO Creator](https://github.com/telekom-security).
...
The TrapPot Universal Installer will upgrade the system and install all required TrapPot dependencies.
...
*Please note*: Cloud providers usually offer adjusted Debian OS images, which might not be compatible with TrapPot. There is no cloud provider support provided of any kind.

<a name="ansible"></a>
### Ansible Deployment
You can find an [Ansible](https://www.ansible.com/) based TrapPot deployment in the [`cloud/ansible`](cloud/ansible) folder.  
...
It first creates all resources (security group, network, subnet, router), deploys one (or more) new servers and then installs and configures TrapPot on them.
...
This can be used to launch a virtual machine, bootstrap any dependencies and install TrapPot in a single step.
...
<a name="firstrun"></a>
## First Run
The installation requires very little interaction, only a locale and keyboard setting have to be answered for the basic linux installation. While the system reboots maintain the active internet connection. The TrapPot installer will start and ask you for an installation type, password for the **tsec** user and credentials for a **web user**. Everything else will be configured automatically. All docker images and other componenents will be downloaded. Depending on your network connection and the chosen installation type, the installation may take some time. With 250Mbit down / 40Mbit up the installation is usually finished within 15-30 minutes.

Once the installation is finished, the system will automatically reboot and you will be presented with the TrapPot login screen. On the console you may login with:
...
<a name="placement"></a>
# System Placement
Make sure your system is reachable through a network you suspect intruders in / from (i.e. the internet). Otherwise TrapPot will most likely not capture any attacks, other than the ones from your internal network! For starters it is recommended to put TrapPot in an unfiltered zone, where all TCP and UDP traffic is forwarded to TrapPot's network interface. However to avoid fingerprinting you can put TrapPot behind a firewall and forward all TCP / UDP traffic in the port range of 1-64000 to TrapPot while allowing access to ports > 64000 only from trusted IPs.
...
In case you need external Admin UI access, forward TCP port 64294 to TrapPot, see below.
In case you need external SSH access, forward TCP port 64295 to TrapPot, see below.
In case you need external Web UI access, forward TCP port 64297 to TrapPot, see below.

TrapPot requires outgoing git, http, https connections for updates (Debian, Docker, GitHub, PyPi), attack submission (ewsposter, hpfeeds) and CVE / IP reputation translation map updates (logstash, listbot). Ports and availability may vary based on your geographical location. Also during first install outgoing ICMP / TRACEROUTE is required additionally to find the closest and fastest mirror to you.

<a name="updates"></a>
# Updates
For the ones of you who want to live on the bleeding edge of TrapPot development we introduced an update feature which will allow you to update all TrapPot relevant files to be up to date with the TrapPot master branch.
**If you made any relevant changes to the TrapPot relevant config files make sure to create a backup first.**

The Update script will:
 - **mercilessly** overwrite local changes to be in sync with the TrapPot master branch
 - upgrade the system to the packages available in Debian (Stable)
 - update all resources to be in-sync with the TrapPot master branch
 - ensure all TrapPot relevant system files will be patched / copied into the original TrapPot state
...
## Maintenance
TrapPot is designed to be low maintenance. Basically, there is nothing you have to do but let it run.

If you run into any problems, a reboot may fix it :bowtie:

If new versions of the components involved appear new docker images will be created and distributed. New images will be available from docker hub and downloaded automatically to TrapPot and activated accordingly.  

<a name="submission"></a>
## Community Data Submission
TrapPot is provided in order to make it accessible to all interested in honeypots. By default, the captured data is submitted to a community backend. This community backend uses the data to feed [Sicherheitstacho](https://sicherheitstacho.eu).
You may opt out of the submission by removing the `# Ewsposter service` from `/opt/tpot/etc/tpot.yml`:
1. Stop TrapPot services: `systemctl stop tpot`
...
```
4. Start TrapPot services: `systemctl start tpot`
...
As an Opt-In it is now possible to also share TrapPot data with 3rd party HPFEEDS brokers.  
If you want to share your TrapPot data you simply have to register an account with a 3rd party broker with its own benefits towards the community. You simply run `hpfeeds_optin.sh` which will ask for your credentials. It will automatically update `/opt/tpot/etc/tpot.yml` to deliver events to your desired broker.
...
You are always invited to participate in development on our [GitHub](https://github.com/trapmn/trappot) page.
...
<a name="faq"></a>
# FAQ
Please report any issues or questions on our [GitHub issue list](https://github.com/trapmn/trappot/issues), so the community can participate.

<a name="contact"></a>
# Contact
The software is provided **as is** in a Community Edition format. TrapPot is designed to run out of the box and with zero maintenance involved. <br>
We hope you understand that we cannot provide support on an individual basis. We will try to address questions, bugs and problems on our [GitHub issue list](https://github.com/trapmn/trappot/issues).

<a name="licenses"></a>
# Licenses
The software that TrapPot is built on uses the following licenses.
...
<a name="credits"></a>
# Credits
Without open source and the fruitful development community (we are proud to be a part of), TrapPot would not have been possible! Our thanks are extended but not limited to the following people and organizations:
...
<a name="staytuned"></a>
# Stay tuned ...
A new version of TrapPot is released about every 6-12 months, development has shifted more and more towards rolling releases and the usage of `/opt/tpot/update.sh`.

<a name="testimonial"></a>
# Testimonials
One of the greatest feedback we have gotten so far is by one of the Conpot developers:<br>
***"[...] I highly recommend TrapPot which is ... it's not exactly a swiss army knife .. it's more like a swiss army soldier, equipped with a swiss army knife. Inside a tank. A swiss tank. [...]"***<br>
